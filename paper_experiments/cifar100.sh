...
function run_fedavg() {
  echo "############################################## Running FedAvg ##############################################"
  alpha="$1"
  python main.py -dataset cifar100 --num-rounds 10 --eval-every 10 --batch-size 128 --num-epochs 1 --clients-per-round 5 -model resnet20 -lr 0.1 --weight-decay 0.0001 -device cuda:0 -algorithm fedavg --server-lr 1 --server-opt sgd --num-workers 0 --where-loading init -alpha ${alpha}
}
